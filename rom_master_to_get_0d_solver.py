...
def mkdir(path, name):
    ...
    #         new_folder_path = new_folder_path + "_" + "new"
    #     os.makedirs(new_folder_path, exist_ok=True)
...
def setup_GT_ROM_parameters(path,model_name,order):
    if order == 1:
        ...
        Params.inflow_input_file = os.path.join(path,'gt_inflow_files','inflow_1d.flow')
        Params.model_order = 0
        Params.solver_output_file = 'GT_0d_solver_output.in' # need this to write out the solver file
        Params.model_name = model_name
        Params.outflow_bc_type = 'rcr' #rcr, resistance or coronary vmr 3d sim uses this and it calls 
        Params.outflow_bc_file =  os.path.join(path,'gt_inflow_files\\')
        # create own .dat file? 
...
def reparse_solver_input_use_martins_1d(gt_file, pred_file):
...
def set_path_name():
    ...
    result_master_folder = 'C:\\Users\\bygan\\Documents\\Research_at_Cal\\Shadden_lab_w_Numi\\2024_spring\\pipeline_testing_master\\for_sb3c_0D'
    ...
    return result_master_folder, svproject_path, Numi_model_path, gt_cl_path, martin_1d_input_path, martin_0d_input_path

def set_path_name2():
    martin_0d_result_path = 'C:\\Users\\bygan\\Documents\\Research_at_Cal\\Shadden_lab_w_Numi\\Martin_result\\results_0d'
    return martin_0d_result_path

def main():
    result_master_folder, svproject_path, Numi_model_path, gt_cl_path, martin_1d_input_path, martin_0d_input_path = set_path_name()
    
    
    for filename in os.listdir(Numi_model_path):  #iterate through all the models in the folder
        result_master_folder, svproject_path, Numi_model_path, gt_cl_path, martin_1d_input_path, martin_0d_input_path = set_path_name()
        martin_0d_result_path = set_path_name2()

        # print('debug: filename ' + filename)
        pred_surf_path = os.path.join(Numi_model_path, filename)
        # print('debug: pred_surf_path' + pred_surf_path)
        check_exist = os.path.join(result_master_folder,filename[:-4])
        while os.path.exists(check_exist):
            check_exist = check_exist + '_new'
        
        os.makedirs(check_exist)
        specific_folder = check_exist
        ...
        # boxcut, fill, remesh the ML model
        #
        #########################################################################
        box_scale = 4
        try:
        ...
        ##########################################################################
        #
        # at this moment, we have created the remeshed model
        # we need to export the boundary faces for simulation
        #
        ##########################################################################
        ...
        # we have dir_a (gt) and dir_b(ML) containing matching caps and unmatched caps (DOES NOT INCLUDE INLET)
        ...
        def captured_more_than_2_outlets(matched_caps_lst):
            ...
            for i in matched_caps_lst:
                if i[1] != None: # if [gt_cap,pred_cap], not [gt_cap,None]
                    count += 1
        ...
        if hi == []: # means pred surf captures all gt caps! result is given
            
            # copy the 1d input file and results to the specific folder
            #GT_0D_result_path = mkdir(specific_folder,'GT_0D_results')
            copy_file(martin_0d_input_path, filename[:-4]+'_0d.in', specific_folder)
            copy_file(martin_0d_result_path, filename[:-4]+'.npy', specific_folder)
            #rename GT input file by adding GT to the front
            os.rename(os.path.join(specific_folder, filename[:-4]+'_0d.in'), os.path.join(specific_folder, 'GT_'+filename[:-4]+'_0d.in'))
            copy_file(gt_cl_path, filename, specific_folder)
            os.rename(os.path.join(specific_folder, filename), os.path.join(specific_folder,'extracted_GT_sim_centerlines_copied.vtp')) 
            ...
            GT_Params = setup_GT_ROM_parameters(specific_folder,filename,0)
            ...
            #reparse_solver_input_use_martins_1d(martin_1d_input, os.path.join(specific_folder,'GT_1d_solver_output.in'))
        
            gt_input_file_path = os.path.join(specific_folder,'GT_0d_solver_output.in')
            #mkdir(specific_folder,'GT_0D_results')
            reparse_solver_input_use_martins_0d(martin_0d_input, gt_input_file_path)
                
            
        #################################################################   
        # 
        #    done with GT sim! (whether it is run the existing one or the modified one)
        #
        #################################################################
        # #test
        ...
        ############################################################################################################
        #
        # determine which caps are not captured by the predicted surface: MERGE THEM INTO the WALL
        #
        ############################################################################################################
...
# write_to_file('C:\\Users\\bygan\\Documents\\Research_at_Cal\\Shadden_lab_w_Numi\\2023_summer_research\\ROM results\\inflow_files\\rcrt_from_parsing.dat',lines)
        ...
        PRED_Params = setup_PRED_ROM_parameters(specific_folder,filename,0)
       ...
       # see if centerline file exiset
        ...
        pdb.set_trace() 
        ###################### run 0d use a seperate file
        # Pred_zerod_Params = setup_PRED_ROM_parameters(specific_folder,filename,0)
        # msh.generate(Pred_zerod_Params,Cl)

        # # parse solver input file (martin_0d_input) so that it matches with Martin's
        reparse_solver_input_use_martins_0d(martin_0d_input, os.path.join(specific_folder,'PRED_0d_solver_output.in'))
        
        
        print('model '+ filename+' completed move on to the next model!!')
        continue
