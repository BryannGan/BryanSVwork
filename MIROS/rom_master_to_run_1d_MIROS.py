...
def setup_GT_ROM_parameters_our_area(path,model_name,order):
    if order == 1:
        Params = Parameters() # put everything is params
        Params.density = 1.06
        Params.output_directory = path
        Params.boundary_surfaces_dir = os.path.join(path,'GT_sim_boundary_faces')
        Params.inlet_face_input_file = 'inlet.vtp'
        Params.centerlines_output_file = os.path.join(path,'modifed_gtcl_our_area.vtp')
        Params.surface_model = os.path.join(path,'modified_gt_surf_for_sim.vtp')
        Params.inflow_input_file = os.path.join(path,'gt_inflow_files','inflow_1d.flow')
        Params.model_order = 1
        Params.solver_output_file = 'GT_1d_solver_output_our_area.in' # need this to write out the solver file
        Params.model_name = model_name
        Params.outflow_bc_type = 'rcr' #rcr, resistance or coronary vmr 3d sim uses this and it calls 
        Params.outflow_bc_file =  os.path.join(path,'gt_inflow_files\\')
        # create own .dat file? 
...
def main():
    ...
    for filename in os.listdir(Numi_model_path):  #iterate through all the models in the folder
        ...
        if hi == []: # means pred surf captures all gt caps! result is given, run martin's 1d solver file
            ...
            ##########
            ########## switch centerlinesectionarea to shoelace area
            ##########
            
            # delete_attributes('CenterlineSectionArea',gtclpd)
            # CenterlineSectionArea,lst_too_small,twod_coord_dict,master_area = get_CenterlineSectionArea_ourway(gtclpd,read_geo(gt_model_path))
            # add_attributes('CenterlineSectionArea',CenterlineSectionArea,gtclpd)
            # write_polydata(os.path.join(specific_folder, 'modifed_gtcl_our_area.vtp'), gtclpd)
            # GT_Params = setup_GT_ROM_parameters_our_area(specific_folder,filename,1)
            # Cl = Centerlines()
            # modified_gt_cl_path = os.path.join(specific_folder, 'modifed_gtcl_our_area.vtp')
            # Cl.read(GT_Params, modified_gt_cl_path)
            # mkdir(specific_folder,'GT_sim_boundary_faces')
            ...
            copy_file(martin_1d_input_path, filename[:-4]+'_1d.in', OneD_result_path)
